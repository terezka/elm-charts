...
customTick tick =
    let
        length = if isOdd tick then 7 else 10

        color = if isOdd tick then "#e4e3e3" else "#b9b9b9"
    in
        Svg.line
            ...
            if tick == 0
                then ""
                    ...
                    if orientation == X then 
                        if isOdd tick
                            then ""
                            else (toString tick) ++ " t"
                    else (toString tick) ++ " °C"

        style =
            if orientation == X then "text-anchor: middle;" else "text-anchor: end;"

        displacement =
            if orientation == X then "0, 27" else "-10, 5"
    in
        Svg.text'
...
composedChart =
    plot
        [ size ( 600, 250 ), padding (40, 40) ]
        [ verticalGrid [ gridTickList [ -20, 20, 40, 60 ], gridStyle [ ( "stroke", "#e2e2e2" ) ] ]
